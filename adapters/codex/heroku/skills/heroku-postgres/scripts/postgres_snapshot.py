#!/usr/bin/env python3
"""Read-only Heroku Postgres snapshot helper."""
...
def main() -> None:
    ...
    payload = {
        ...
        "notes": [
            ...
            "Pass --app to collect Postgres-related output when authentication is available.",
        ],
    }
    ...
    if heroku_path and args.app:
        payload["inspections"].extend(
            [
                run(["heroku", "pg:info", "-a", args.app]),
                run(["heroku", "pg:backups", "-a", args.app]),
                run(["heroku", "pg:credentials", "-a", args.app]),
            ]
        )
