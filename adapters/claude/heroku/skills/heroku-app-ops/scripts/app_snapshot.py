#!/usr/bin/env python3
"""Read-only Heroku app snapshot helper."""
...
def main() -> None:
    ...
    payload = {
        ...
        "notes": [
            ...
            "Pass --app to collect app-specific output when authentication is available.",
        ],
    }
    ...
    if heroku_path and args.app:
        payload["inspections"].extend(
            [
                ...
                run(["heroku", "releases:info", "-a", args.app]),
                run(["heroku", "maintenance", "-a", args.app]),
            ]
        )
