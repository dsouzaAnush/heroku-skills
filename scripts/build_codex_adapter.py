#!/usr/bin/env python3
"""Build a self-contained Codex plugin bundle from the portable skills repo."""
...
def build_bundle(repo_root: Path, output_dir: Path) -> Path:
    ...
    template_path = plugin_root / ".codex-plugin" / "plugin.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output directory for the Codex adapter bundle",
    )
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output
        else repo_root / "dist" / "codex" / "heroku"
    )
